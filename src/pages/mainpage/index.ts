...
export async function getServerSideProps(context: PropsType) {
  ...
  const result = await axios.get(
    `${CONFIG.API_URL}/movie/popular?api_key=${CONFIG.API_KEY}&language=ko-KR&sort_by=popularity.desc&release_date.gte=${lastDay}&page=${currPage}&release_date.lte=${today}&include_adult=false`
  )
